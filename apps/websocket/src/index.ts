...
wss.on(
  "connection",
  function connection(
    ws: WebSocket & { roomId?: number; userId?: number; senderName?: string },
...
async function handleCreateRoom(
  ws: WebSocket & { roomId?: number; userId?: number },
  roomName: string,
  ...
  ws.userId = creatorId;
  ws.send(
    JSON.stringify({ type: "system", message: `Room created: ${roomName}` }),
  );
}
...
async function handleJoinRoom(
  ...
  if (!room) {
    ws.send(
      JSON.stringify({
        type: "error",
        message: `Room does not exist: ${roomName}`,
      }),
    );
    return;
  }
...
async function handleSendMessage(
  ...
  if (!roomId || !validateMessage(content)) {
    ws.send(
      JSON.stringify({
        type: "error",
        message: "Invalid message or room not joined",
      }),
    );
    return;
  }
...
function broadcastMessage(
  ...
  wss.clients.forEach((client) => {
    if (
      ...
      client.send(
        JSON.stringify({ type: "message", content, userId, senderName }),
      );
    }
  });
}
