...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!passkey) {
      return NextResponse.json({ msg: "Passkey is required" }, { status: 400 });
    }
    if (!validateRoomName(passkey) || passkey.length < 10) {
      return NextResponse.json({ msg: "Passkey is Invalid" }, { status: 411 });
    }
    if (!userId) {
    ...
    if (!Room) {
      return NextResponse.json({ msg: "No Room Existed" }, { status: 400 });
    }
