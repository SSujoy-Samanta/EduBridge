...
export async function POST(req: NextRequest) {
  ...
  if (!roomName) {
    return NextResponse.json({ msg: "Room name is required" }, { status: 400 });
  }
  ...
  if (!parseRoom.success || !creatorId) {
    return NextResponse.json(
      {
        msg: "Invalid inputs",
        errors: parseRoom.error?.errors || "Creator ID is missing",
      },
      { status: 411 },
    );
  }
  ...
  try {
    ...
    let room = await client.room.findUnique({
      where: { name: parseRoom.data.roomName },
    });
    ...
    return NextResponse.json(
      {
        msg: "Could not create room & Internal Server Error",
        error: error.message,
      },
      { status: 500 },
    );
