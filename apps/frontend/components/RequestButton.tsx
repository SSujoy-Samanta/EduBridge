...
export const RequestButton = ({ userId, receiverId }: { userId: number; receiverId: number }) => {
    const setNotification = useSetRecoilState(notificationState);
    const [trigger, setTrigger] = useState<boolean>(false);
    const { status, actualSender, actualReceiver } = useStatus(userId, receiverId, trigger);
    ...
    const handleCancelRequest = async () => {
        ...
        try {
            ...
            toggleTrigger(); // Toggle trigger to re-fetch status
            setNotification({ msg: "Request Canceled Successfully", type: "success" });
        } catch (error: any) {
            setNotification({ msg: error.response?.data.msg || "Error canceling request", type: "error" });
        }finally {
            setLoading(false);
        }
    };
    ...
    const handleUnfriend = async () => {
        ...
        try {
            ...
            setNotification({ msg: error.response?.data.msg || "Error while unfriending", type: "error" });
        }finally {
            setLoading(false);
        }
    };
    ...
    return (
        <div className="flex flex-col gap-2 w-full">
            {status === null ? (
                <button className="rounded p-2 bg-blue-600" onClick={handleSendRequest} disabled={loading}>
                    CONNECT
                </button>
            ) : status === 'PENDING' && userId === actualSender ? (
                <button className="rounded p-2 bg-red-600" onClick={handleCancelRequest} disabled={loading}>
                    Cancel Request
                </button>
            ) : status === 'PENDING' && userId === actualReceiver ? (
                <div className="flex flex-col gap-2">
                    <button className="rounded p-2 bg-blue-600" onClick={handleAcceptRequest} disabled={loading}>
                        Accept
                    </button>
                    <button className="rounded p-2 bg-red-600" onClick={handleCancelRequest} disabled={loading}>
                        Decline
                ...
                <button className="rounded p-2 bg-slate-800" onClick={handleUnfriend} disabled={loading}>
                    Unfriend
                </button>
            )}
        </div>
