  ...
  useEffect(() => {
    if (id && room && WSURL) {
      const newSocket = new WebSocket(
        `${WSURL}?roomName=${room.split("#")[1]}&userId=${id}`,
      );
...
export const VideoApp = ({
        ...
        setSendingPc((pc) => {
          if (pc) {
            if (pc.signalingState === "have-local-offer") {
              ...
              console.warn(
                "Peer connection is in stable state. Cannot set remote answer SDP at this time.",
              );
            } else {
  ...
  return (
    <div className="w-full pt-10 flex flex-col justify-center items-center gap-3">
      <h1 className="text-rose-700 border rounded-sm p-2 text-center border-slate-600 font-bold">
        Room: <span className="text-teal-500">{room.split("#")[0]}</span>
      </h1>
        ...
        <div className="flex flex-col justify-start items-center w-full  border-2 rounded-sm border-slate-800">
          {!lobby ? (
            ...
            <div className="w-full h-full md:h-full xxs:h-64  flex justify-center flex-col">
              <div className="font-light bg-sky-950 w-full  text-center p-1 text-red-700 ">
                {" "}
