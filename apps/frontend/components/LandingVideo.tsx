...
export const Landing = () => {
  ...
  const toggleAudio = () => {
                   ...
                   <div className="w-full flex flex-col justify-center gap-2">
                        ...
                        {creat && 
                            <div className="flex justify-start items-end gap-2 w-full ">
                                ...
                                <InputBox label="Room Name" placeholder="Enter Room Name" onChange={(e) => setRoom(e.target.value)}/>
                                <button className="border border-slate-700 p-2 rounded-md" onClick={async() =>{
                                    if(room){
                                        setNotification({msg:`${room} created successfully`,type:"success"});
                                        const x=await passkeyGen(room);                                        setPasskey(x)
                                        setCreate(false);
                                    } else{
                                        setNotification({msg:"Invalid Room name",type:"error"});
                                    }
                        ...
                        {
                            passkey && <div className="flex items-center gap-2 font-mono text-slate-800">
                    ...
                    <div className="flex justify-start items-end gap-2 w-full ">
                        ...
                        <InputBox label="Join Meeting" placeholder="Enter The Passkey" onChange={(e) => setKey(e.target.value)}/>
                        <button className="border border-slate-700 p-2 rounded-md" onClick={() =>{
                            if (key && key.includes('#') && key.split('#')[1]?.length === 10) {
                                setNotification({ msg: `Joining the Room: ${key.split('#')[0]}`, type: "success" });
                                setJoined(true);
                            } else {
    ...
    return <VideoApp room={key} localAudioTrack={localAudioTrack} localVideoTrack={localVideoTrack} toggleVideo={toggleVideo} toggleAudio={toggleAudio}/>;
};
